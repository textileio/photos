...
export function * prepareAndAdd(path: string, threadId: string, caption?: string) {
  const preparedFiles: IMobilePreparedFiles = yield call(Textile.files.prepareByPath, path, threadId)
  const block: IBlock = yield call(Textile.files.add, preparedFiles.dir, threadId, caption)
