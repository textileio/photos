...
const actions = {
  ...
  addExternalInvite: createAction('ADD_EXTERNAL_THREAD_INVITE', resolve => {
  ...
  acceptExternalInvite: createAction('ACCEPT_EXTERNAL_THREAD_INVITE', resolve => {
...
export function reducer (state: ThreadsState = initialState, action: ThreadsAction): ThreadsState {
  switch (action.type) {
    ...
    case getType(actions.addExternalInviteError): {
      ...
      const { error } = action.payload
      return { ...state, links: { ...state.links, error } }
    }
    ...
    case getType(actions.acceptExternalInviteSuccess): {
      ...
      return { ...state, invites: undefined }
    }
    case getType(actions.acceptExternalInviteError): {
      ...
      const { error } = action.payload
      return { ...state, invites: { ...state.invites, error } }
    }
    default:
      return state
  }
