...
const actions = {
  ...
  refreshNotificationsSuccess: createAction('REFRESH_NOTIFICATIONS_SUCCESS', resolve => {
    return (notifications: TT.Notification[]) => resolve({notifications})
  }),
  ...
  newNotificationRequest: createAction('NEW_NOTIFICATION_REQUEST', resolve => {
    return (notification: TT.Notification) => resolve({notification})
  }),
  notificationEngagement: createAction('NOTIFICATION_ENGAGEMENT', resolve => {
    return (engagement: TT.NotificationEngagement) => resolve({ engagement: engagement })
  }),
...
export type NotificationsState = {
  readonly notifications: ReadonlyArray<TT.Notification>,
  refreshing: boolean
}
