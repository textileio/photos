...
export const uploadProgress = createAction('file-sync/UPLOAD_PROGRESS', (resolve) => {
  return (id: string, progress: number) => resolve({ id, progress })
})

export const uploadComplete = createAction('file-sync/UPLOAD_COMPLETE', (resolve) => {
  return (id: string, responseCode: string, responseBody: string) => resolve({ id, responseCode, responseBody })
})
