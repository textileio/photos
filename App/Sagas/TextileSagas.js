...
export function * createNode ({path}) {
  try {
    ...
    const addDefaultThreadSuccess = yield call(TextileNode.addThread, "default", "")
    const addAllThreadSuccess = yield call(TextileNode.addThread, Config.ALL_THREAD_NAME, Config.ALL_THREAD_MNEMONIC)
...
export function * getPhotoBlocks ({thread}) {
  try {
    const items = yield call(TextileNode.getPhotoBlocks, null, -1, thread)
    let data = []
